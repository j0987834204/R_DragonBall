...
# 篩選Fundation(地基類型), Functional(房屋損毀狀況), Bedroom, Bath, Kitchen的欄位 
train_other <- select(train_non_null, matches("Foundation|Functional|Bedroom|Bath|Kitchen|SalePrice"))
train_other %>%
  gather(-SalePrice, key = "var", value = "value") %>%
  ggplot(aes(x = value, y = SalePrice)) +   # Plot the values
  facet_wrap(~ var, scales = "free") +   # In separate panels
