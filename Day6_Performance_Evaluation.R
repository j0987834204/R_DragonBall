...
# Windows :  
setwd("C:/Users/LC/Desktop/R_DragonBall")  
...
test0 = read.csv("test_new1.csv")

test <- test0 %>% dplyr::select(X1stFlrSF, TotalBsmtSF, 
                               YearBuilt, LotArea, Neighborhood, GarageCars, GarageArea, GrLivArea_stand, 
                               MasVnrArea_stand, LotFrontage_log, is_Fireplace, TotalBathrooms, TotalSF_stand)

cat_index <- which(sapply(test, class) == "factor")
test[cat_index] <- lapply(test[cat_index], as.numeric)

dtest <- xgb.DMatrix(data = as.matrix(test))

pred_xgb_test = predict(xgb_base, dtest)

realprice_test = exp(pred_xgb_test)


write.table(realprice_test,file="predict_test.csv",sep=",",row.names=F, na = "NA",  col.names=FALSE)
